...
namespace Eventos.Application
{
  public class RedeSocialService : IRedeSocialService
  {
    ...
    public async Task<RedeSocialDto[]> SaveByPalestrante(int palestranteId, RedeSocialDto[] models)
    {
      try
      {
        var redeSocials = await _redeSocialPersist.GetAllByEventoIdAsync(palestranteId);
        if (redeSocials == null) return null;
    ...
    public async Task<bool> DeleteByPalestrante(int palestranteId, int redeSocialId)
    {
      try
      {
        ...
        if (redeSocial == null) throw new Exception("Rede Social pro Palesteante para delete não encontrado.");
    ...
    public async Task<RedeSocialDto[]> GetAllByPalestranteIdAsync(int palestrante)
    {
      try
      {
        var redeSocial = await _redeSocialPersist.GetAllByEventoIdAsync(palestrante);
        if (redeSocial == null) return null;
